...
public class SimulationBenchmark : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        iterationsPerSecText.text = "0 it/s";
    ...
    {
        // Smoothly rotate the camera around the given position
        Camera.main.transform.RotateAround(position, Vector3.up, speed * Time.deltaTime);

        // Calculate the desired distance from the center
        float minDistance = 20f;
        float maxDistance = 40f;
        float oscillationSpeed = 0.5f;
        float t = (Mathf.Sin(Time.time * oscillationSpeed) + 1f) / 2f; // oscillates between 0 and 1
    ...
    void UpdatePerformanceMetrics()
    {
        if (Time.time > nextUpdate)
        {
            ...
            iterationsPerSecList.Add(iterationsPerSec);
        }
    }
    ...
    private void OnApplicationQuit()
    {
        ...
        double averageIterationsPerSec = iterationsPerSecList.Count > 0 ? iterationsPerSecList.Average() : 0;
        // Save the average iterations per second to temporary file in InfinityBench 
        string basePath;
        if (SystemInfo.operatingSystemFamily == OperatingSystemFamily.MacOSX)
        {
            basePath = System.IO.Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.Personal),
            "Library", "Application Support", "InfinityBench");
        }
        else if (SystemInfo.operatingSystemFamily == OperatingSystemFamily.Windows)
        {
            string appData = Environment.GetEnvironmentVariable("APPDATA") ??
                     System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "AppData", "Roaming");
            basePath = System.IO.Path.Combine(appData, "InfinityBench");
        }
        else
        {
            // Default to Documents/InfinityBench for other OS
            basePath = System.IO.Path.Combine(
